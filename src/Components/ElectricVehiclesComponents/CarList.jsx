      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-4 md:gap-6 place-items-center">
        {cars.map((car) => (
          <div
            onClick={
              () => {
                navigate(`/electric-vehicles/details-vehicle/${car.id}/${car.car_name}/${car.car_model}/${car.car_year}`,
                  { state: car });
              }}
            key={car.id} className="flex flex-col items-center w-3xs lg:w-3xs xl:w-md 2xl:w-xs md:w-4xs max-w-[320px] p-4">
            <div
